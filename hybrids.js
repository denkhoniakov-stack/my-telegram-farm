            ...
            try {
                ...
                if (gameState.farmers && Array.isArray(gameState.farmers)) {
                    gameState.farmers.forEach(farmer => {
                        // Проверка на активность (true или 'true') и тип бонуса
                        if ((farmer.isActive === true || farmer.isActive === 'true') && farmer.bonusType === 'lab') {
                            // Преобразуем значение в число, чтобы избежать ошибок сложения строк
                ...
                if (labSpeedBonus > 0) {
                    // Ограничиваем бонус до 99%, чтобы время не стало 0 или отрицательным
                    if (labSpeedBonus > 99) labSpeedBonus = 99;
                    duration = duration * (1 - labSpeedBonus / 100);
                }
            } catch (e) {
                console.error("Ошибка расчета бонуса фермеров:", e);
                // Если произошла ошибка, duration останется стандартным (stats.growTime)
            }
            // ------------------------------------------
